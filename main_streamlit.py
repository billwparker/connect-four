...
def backpropagate(node, winner):
    ...
    while node is not None:
        node.visits += 1
        # If the winning player is the parent’s player, increment wins.
        # Another approach: if winner == get_next_player(node.current_player)
...
def display_board(board, col1):
    """
    Display the board in Streamlit using a fixed-pitch font, ensuring rows are on separate lines.
    """
    board_str = ""
    for row in range(ROWS):
        row_str = "| "
